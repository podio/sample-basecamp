...
class Basecamp
  ...
  class Message < Resource
    ...
    def comments
      response = Basecamp.connection.get("/posts/#{id}/comments.xml")
      xml = XmlSimple.xml_in(response.body)
      if xml['count'].to_i != 0
        comments = xml['comment'].map {|c|
          {:author=>c['author-id'].first['content'], :body => c['body'].join('<br>'),
            :id => c['id'].first['content'].to_i}
        }
      else
        comments = nil
      end
  ...
  class TodoItem < Resource
    ...
    def comments(options = {})
      response = Basecamp.connection.get("/todo_items/#{id}/comments.xml")
      xml = XmlSimple.xml_in(response.body)
      if xml['count'].to_i != 0
        @comments ||= xml['comment'].map {|c|
          # p c
  ...
  def milestone_comments(id)
      response = Basecamp.connection.get("/milestones/#{id}/comments.xml")
      xml = XmlSimple.xml_in(response.body)
      if xml['count'].to_i != 0
        comments = xml['comment'].map {|c|
          {:author=>c['author-id'].first['content'], :body => c['body'].join('<br>'),
